...
describe('myAccounts feature (always enabled for self-hosted deployments)', () => {
  it('useIsEnabled returns true unconditionally', () => {
    const { MyAccountsFeature } = jest.requireActual('@/features/myAccounts')
    const { result } = renderHook(() => MyAccountsFeature.useIsEnabled())
    expect(result.current).toBe(true)
  })

  it('has correct name', () => {
    const { MyAccountsFeature } = jest.requireActual('@/features/myAccounts')
    expect(MyAccountsFeature.name).toBe('myAccounts')
  })
})
